 ...
 * chỉ khác ở chỗ KHÔNG cần preview ảnh phía sau: cửa sổ dialog toàn màn hình đã tự có sẵn nền
 * blur/màu đặc phủ kín phía sau contentView rồi (DialogHelper.setWindowBlurBg gọi lúc mở dialog),
 * nên chỉ cần trượt contentView sang phải là lộ ra đúng nền đó, không cần chụp/giữ bitmap riêng.
 *
...
class DialogSwipeBackHelper(
    ...
    companion object {
         ...
         */
        fun bind(dialog: Dialog, contentView: View, onBack: () -> Unit): DialogSwipeBackHelper? {
            val window = dialog.window ?: return null
            val helper = DialogSwipeBackHelper(contentView.context, contentView, onBack)
        ...
        when (ev.actionMasked) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (dragging) {
                    ...
                    contentView.translationX = clampedDx
                    return true
                }
                return false
            }
    ...
    private fun animateTo(target: Float, velocityX: Float, onEnd: (() -> Unit)?) {
        ...
        settleAnimator = ValueAnimator.ofFloat(start, target).apply {
            ...
            addUpdateListener { contentView.translationX = it.animatedValue as Float }
