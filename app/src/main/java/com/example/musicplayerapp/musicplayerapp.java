...
public class MusicService extends Service {
    ...
    public void onCreate() {
        ...
        player.setOnCompletionListener(mp -> nextSong()); // Chuyển bài khi hết bài hiện tại
    }
    ...
    private void nextSong() {
        if (currentIndex + 1 < playlist.size()) {
            ...
            stopForeground(true); // Nếu hết playlist thì dừng dịch vụ
        }
    }
    ...
    private void showNotification(String status, String path) {
        String fileName = path.substring(path.lastIndexOf('/') + 1);
