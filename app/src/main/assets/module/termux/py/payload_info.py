...
_runtime_version.ValidateProtobufRuntimeVersion(
    _runtime_version.Domain.PUBLIC, 5, 27, 2, "", "update_metadata.proto"
)
...
DeltaArchiveManifest = _sym_db.GetSymbol("chromeos_update_engine.DeltaArchiveManifest")
...
class RemoteZipFile:
    ...
    __slots__ = ("url", "size", "pos", "_timeout")

    def __init__(self, url: str, timeout: int = _REQUEST_TIMEOUT) -> None:
        self.url = url
...
@contextmanager
def loading_animation(message: str = "Reading payload.bin") -> Generator:
    ...
    def _spin() -> None:
        for idx in range(10**9):
            if stop.is_set():
                break
            sys.stdout.write(f"\r{message} {_SPINNER[idx % 3]}")
            sys.stdout.flush()
            time.sleep(0.3)
...
def print_manifest(m) -> None:
    ota_type = "A/B"
    if m.HasField("dynamic_partition_metadata"):
        dpm = m.dynamic_partition_metadata
        if getattr(dpm, "vabc_enabled", False):
            ota_type = "Virtual A/B Compression"
        elif getattr(dpm, "snapshot_enabled", False):
            ota_type = "Virtual A/B"

    print(f"\nArchitecture: {ota_type}")

    if not m.HasField("dynamic_partition_metadata"):
        print("\nDynamic Partition Metadata Not Found")
        return

    groups = m.dynamic_partition_metadata.groups
    for i, g in enumerate(groups):
        physical_size = round(g.size / _CHUNK_SIZE) * _CHUNK_SIZE
        print(f"\nGroup: {g.name}")
        print(f"\nSize: {physical_size / _GIB:.2f}GB ({physical_size})")
        if g.partition_names:
            print(f"\nPartitions: " + " ".join(g.partition_names) + "\n")
        if i < len(groups) - 1:
            print()


def parse_payload_file(f: IO[bytes]):
    if f.read(4) != _PAYLOAD_MAGIC:
        raise ValueError("Invalid payload.bin: bad magic bytes")
    version = struct.unpack(">Q", f.read(8))[0]
    manifest_size = struct.unpack(">Q", f.read(8))[0]
    if version == 2:
        f.read(4)
    manifest = DeltaArchiveManifest()
    manifest.ParseFromString(f.read(manifest_size))
    return manifest
...
def main() -> None:
    if len(sys.argv) < 2:
        print(
            f"Usage: python {os.path.basename(sys.argv[0])} <payload.bin|ota.zip|URL>"
        )
        sys.exit(1)

    target = sys.argv[1]
    processor = (
        _process_url if target.startswith(("http://", "https://")) else _process_local
    )

    try:
        with loading_animation():
            manifest = processor(target)
