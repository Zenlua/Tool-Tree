...
class KeepShell(private var rootMode: Boolean = true) {
    ...
    // Giống doCmdSync(), nhưng gọi onLine(dòng) NGAY KHI đọc được 1 dòng output mới từ shell,
    // thay vì chỉ trả kết quả một lần sau khi toàn bộ script (kể cả các lệnh chạy chậm như
    // sleep) đã kết thúc. Dùng cho trang có process=true để hiện từng khối cấu hình ngay khi
    // shell vừa xuất xong khối đó - xem PageConfigSh.executeStreaming().
    // Vẫn trả về toàn bộ output (giống doCmdSync) để nơi gọi có thể fallback khi cần.
    fun doCmdStreaming(cmd: String, onLine: (String) -> Unit): String {
        if (mLock.isLocked && enterLockTime > 0 && System.currentTimeMillis() - enterLockTime > LOCK_TIMEOUT) {
            tryExit()
            Log.e("doCmdStreaming-Lock", "Thread wait timeout ${System.currentTimeMillis()} - $enterLockTime > $LOCK_TIMEOUT")
        }
        getRuntimeShell()
        ...
        try {
            ...
            while (reader != null) {
                ...
                if (line == null) {
                    ...
                    val tail = line.substringBefore(endTag)
                    if (tail.isNotEmpty()) {
                        shellOutputCache.append(tail)
                        onLine(tail)
                    }
                    break
                } else if (line.contains(startTag)) {
                    shellOutputCache.clear()
                    val head = line.substring(line.indexOf(startTag) + startTag.length)
                    shellOutputCache.append(head)
                    unstart = false
                    if (head.isNotEmpty()) {
                        onLine(head)
                    }
