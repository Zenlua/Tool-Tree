...
class PageConfigSh(private var activity: Activity, private var pageConfigSh: String, private var parentConfig: PageNode?) {
    ...
    // Header TOML ở TOP-LEVEL (vd "[[group]]", "[[menu]]"...) - phân biệt với header lồng bên
    // trong như "[[group.action]]" (có dấu chấm, không khớp regex này). Mỗi lần khớp là dấu
    // hiệu 1 khối cấu hình mới bắt đầu trong output streaming.
    private val topLevelHeaderRegex = Regex("^\\[\\[(group|text|switch|picker|action|page|download|editor|resource|menu|fab)]]$")
    ...
    // theo) xuất hiện ĐẦY ĐỦ là parse & trả về ngay qua onNodeReady - KHÔNG đợi các lệnh chạy
    // sau nó (vd sleep) trong script hoàn tất.
    ...
    fun executeStreaming(onNodeReady: ((NodeInfoBase?, Int, Int) -> Unit)? = null): ArrayList<NodeInfoBase>? {
        ...
        ScriptEnvironmen.executeStreamingRoot(activity, pageConfigSh, parentConfig) { line ->
            ...
            if (topLevelHeaderRegex.matches(trimmed)) {
